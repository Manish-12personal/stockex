...
/** IST second for end-of-leg **LTP** (last second of betting window W, 1-based): marketOpen + W * D - 1. */
export function niftyOpenFixSecForWindow(W, gameConfig = {}) {
  const { marketOpenSec } = getEffectiveNiftySessionBounds(gameConfig);
  const D = getNiftyRoundDurationSec(gameConfig);
  return marketOpenSec + W * D - 1;
}

/**
 * IST seconds where window W (1-based) **result** price is fixed: window end + D (15 minutes after window ends).
 */
export function niftyResultSecForWindow(W, gameConfig = {}) {
  const { marketOpenSec } = getEffectiveNiftySessionBounds(gameConfig);
  const D = getNiftyRoundDurationSec(gameConfig);
  return marketOpenSec + W * D - 1 + D;
}
